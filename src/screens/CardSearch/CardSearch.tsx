...
import { CardViewStyle, GridStyles, SearchStyle } from './styles';
...
const CardSearching = (props: BoxProps) => {
  ...
  const biggerScreen = useBreakpointValue({
    base: false,
    sm: false,
    md: true
  })
  ...
  useOutsideClick({
    ref: ref,
    handler: () => !biggerScreen && setSearch('')
  })
  ...
  return (
    <Box as='main' {...props}>
      {!biggerScreen && <>
        ...
        {selectedCard &&
          <SimpleGrid columns={{ sm: 1, md: 2}} spacing='1rem' {...CardViewStyle.SimpleGrid}>
            <Image 
              {...CardViewStyle.Image}
      ...
      {biggerScreen &&
        <Grid {...GridStyles}>
          ...
          <GridItem area='card'>
            <Box>
              {selectedCard &&
                <SimpleGrid columns={{ sm: 1, md: 2}} spacing='1rem' {...CardViewStyle.SimpleGrid} mt={0}>
                  <Image 
                    {...CardViewStyle.Image}
                    src={selectedCard.card_images[0].image_url} 
                    alt={selectedCard.name}
